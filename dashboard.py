...
with tab1:
    ...
    st.markdown("Each draft slot is attributed points via the Fitzgerald-Spielberger chart and expected draft slot comes from Arif Hasan's consensus mock draft board. Adj. Avg maps directly to Grades and is the amount of Fitzgerald-Spielberger points divided by consensus board-ranked picks made by each team. Points and the resulting grade derive from 1) pick # for player relative to mock draft consensus board # 2) trade net on pure pick swap deals.")
